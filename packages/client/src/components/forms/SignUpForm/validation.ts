...
const SIGNUP_FORM_VALIDATION_SCHEMA = yup.object().shape({
  ...
  login: yup.string()
    ...
    .matches(ONLY_LATIN_LETTERS, "Login should use only latin letters and digits")
...
export const validator = (input: RuleObject, value: StoreValue) =>
  yup.reach(SIGNUP_FORM_VALIDATION_SCHEMA, input.field)
    .validate(value)
    .then(_ => Promise.resolve())
    .catch(e => Promise.reject(e));


export const matchPasswords = ({ getFieldValue }) => ({
