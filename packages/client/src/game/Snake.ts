...
export class MySnake {
  ...
  move(x: number, y: number, boost = false) {
    const distanceToMouse = getDistanceBetweenTwoPoints(
      { x: this.x, y: this.y },
      { x, y }
    )
    ...
    const distanceFromHeadToTail = getDistanceBetweenTwoPoints(
      this.segments[0],
      { x: this.x, y: this.y }
    )

    // Если голова змейки не прошла необходимое минимальное расстояние, то элементы хвоста змеи оставляем на тех же местах
  ...
  drawCoordinateLogs(index: number, x: number, y: number) {
    ...
    this.ctx.fillText(
      `[${index}] x: ${Math.ceil(x)} ; y: ${Math.ceil(y)}`,
      10,
      100 + index * 20
    )
  }
  ...
  drawEyes() {
    const deltaAngle = Math.atan2(
      this.y - this.segments[1].y,
      this.x - this.segments[1].x
    )
